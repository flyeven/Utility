...
using Utility.CommonUtility.CryptoService;
...
namespace Utility.CommonUtility.Extension
{
    ...
    /// </summary>
    public sealed class ConfigurationManagerExtension
    {
        ...
        public static string GetDBConnectStr(string databaseName = null)
        {
            ...
            string encryptedFileds = GetStringValue(databaseName + "ENCRYPTED_FIELDS");

            string user = DecryptString(GetStringValue(databaseName + "DB_USER"), encryptedFileds);
            string schema = DecryptString(GetStringValue(databaseName + "DB_DATABASE"), encryptedFileds);
            string IP = DecryptString(GetStringValue(databaseName + "DB_IP"), encryptedFileds);
            string port = DecryptString(GetStringValue(databaseName + "DB_PORT"), encryptedFileds);
            string psw = DecryptString(GetStringValue(databaseName + "DB_PSW"), encryptedFileds);
        ...
        /// <param name="encryptedFileds"></param>
        /// <returns></returns>
        private static string DecryptString(string originalString, string encryptedFileds)
        {
            //加密时，额外处理
            if (string.IsNullOrEmpty(encryptedFileds) || encryptedFileds.Contains(originalString) == false)
                return originalString;

            DECCryptography dec = new DECCryptography("DBDecrypt");
            return dec.DecryptString(originalString);
        }
